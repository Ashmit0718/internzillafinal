...
const api = {
  ...
  forgotPassword: (email) => api.request('/auth/forgot-password', { body: { email } }),
  resetPassword: (token, newPassword) => api.request('/auth/reset-password', { body: { token, newPassword } }),
...
// --- Auth Components (Extracted to AuthPage.jsx in a future step, modified here for now) ---
...
function App() {
  ...
  const handleLogin = (jwt) => { localStorage.setItem('token', jwt); setToken(jwt); navigate('/'); };
  const handleLogout = useCallback(() => { localStorage.removeItem('token'); setToken(null); navigate('/login'); }, [navigate]);
    ...
    <div className="app-wrapper">
      ...
      <main className="container">
        <Routes>
          ...
          <Route path="/*" element={user ? <DashboardContent user={user} isLoadingProfile={isLoadingProfile} hasProfile={hasProfile} onProfileCreated={() => setHasProfile(true)} /> : <Navigate to="/login" />} />
        </Routes>
      </main>
    </div>
  );
...
const DashboardContent = ({ user, isLoadingProfile, hasProfile, onProfileCreated }) => {
  const [currentView, setCurrentView] = useState('DASHBOARD');
  const handleNavigation = (view) => setCurrentView(view);

  if (isLoadingProfile) { return <div className="loading"></div>; }
  if (user.role === 'STUDENT' && !hasProfile) { return <CreateProfilePage onProfileCreated={onProfileCreated} />; }
